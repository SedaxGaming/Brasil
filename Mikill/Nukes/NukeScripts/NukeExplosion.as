...
void onInit(CBlob@ this)
{
	...
	if (!this.exists("flash_distance")) this.set_f32("flash_distance", 99900);
...
void DoExplosion(CBlob@ this)
{
	...
	const f32 invModifier = 10.00f - modifier;

	this.set_f32("map_damage_radius", 900.0f * modifier);

	this.set_Vec2f("explosion_offset", Vec2f(0, 0));
	Explode(this, 192.0f * modifier, 192.0f * (10.00f - modifier));
			...
			if (ticks == 2)
			{
				...
				if (this.getMap().getBlobsInRadius(this.getPosition(), this.get_f32("flash_distance"), @blobs))
				{
					...
					for (int i = 0; i < blobs.length; i++)
					{
						...
						{
							this.server_Hit(blob, blob.getPosition(), Vec2f(), 350.00f, Hitters::bomb, true);
						}
					}
				}
			}
