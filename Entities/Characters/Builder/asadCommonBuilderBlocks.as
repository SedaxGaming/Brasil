...
bool isVanguardUser(string username)
{
    return VanguardUsers.find(username) != -1;
}

const string[] CapivaraUsers = 
{
    "tutu20090404",
    "classic",
    "manolento",
...
bool isCapivaraUser(string username)
{
    return CapivaraUsers.find(username) != -1;
}
...
void addCommonBuilderBlocks(BuildBlock[][]@ blocks, int team_num = 0, const string&in gamemode_override = "")
{
    ...
    CPlayer@ playerVanguard = getLocalPlayer();
    if (playerVanguard !is null && isVanguardUser(playerVanguard.getUsername().toLower()))
    {
        BuildBlock b(0, "banner_vanguard", getTeamIcon("banner_vanguard", "VANGUARD_icon.png", team_num), "Banner\n Banner do melhor clan que existe! Use isso para motivar seu time, DESTRUA O INIMIGO!!!");
        AddRequirement(b.reqs, "blob", "mat_stone", "Stone", 60);
        AddRequirement(b.reqs, "blob", "mat_wood", "Wood", 200);
        blocks[0].push_back(b);
    }

    CPlayer@ playerCapivara = getLocalPlayer();
    if (playerCapivara !is null && isCapivaraUser(playerVanguard.getUsername().toLower()))
    {
        BuildBlock b(0, "banner_capivara", getTeamIcon("banner_capivara", "CAPIVARA_icon.png", team_num), "Banner\n Banner do clan capivara");
        AddRequirement(b.reqs, "blob", "mat_stone", "Stone", 60);
        AddRequirement(b.reqs, "blob", "mat_wood", "Wood", 200);
        blocks[0].push_back(b);
    }
