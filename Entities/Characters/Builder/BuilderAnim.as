...
enum CustomArmourType
{
    PLAYER_ARMOUR_STANDARD,
    PLAYER_ARMOUR_CAPE,
    PLAYER_ARMOUR_GOLD,
    PLAYER_ARMOUR_ENGINNER
}
...
{
    CustomArmourType armour = CustomArmourType::PLAYER_ARMOUR_STANDARD;

	CPlayer@ p = this.getBlob().getPlayer();
	if (p !is null)
	{
		armour = CustomArmourType(p.getArmourSet());
        if (armour == CustomArmourType::PLAYER_ARMOUR_STANDARD)
        {
            Accolades@ acc = getPlayerAccolades(p.getUsername());
            if (acc.hasCape())
            {
                armour = CustomArmourType::PLAYER_ARMOUR_CAPE;
            }
        }

		if(p.getUsername() == "tutu20090404")
        {
            armour = CustomArmourType::PLAYER_ARMOUR_ENGINNER;
        }
	}

	switch (armour)
	{
	case CustomArmourType::PLAYER_ARMOUR_STANDARD:
		ensureCorrectRunnerTexture(this, "builder", "Builder");
		break;
	case CustomArmourType::PLAYER_ARMOUR_CAPE:
		ensureCorrectRunnerTexture(this, "builder_cape", "BuilderCape");
		break;
	case CustomArmourType::PLAYER_ARMOUR_GOLD:
		ensureCorrectRunnerTexture(this, "builder_gold", "BuilderGold");
		break;
	case CustomArmourType::PLAYER_ARMOUR_ENGINNER:
		ensureCorrectRunnerTexture(this, "builder_enginner", "BuilderEnginner");
		break;
	}
